...
> ⚠️ **如果 clone / raw 直连失败（网络被重置）**：告诉 agent 改用 **GitHub API blob 接口**下载（`api.github.com` 的 contents 接口返回 base64，实测在 git clone 和 raw 都走不通的环境下可用）。也可手动跑：`powershell -ExecutionPolicy Bypass -File install.ps1 -FromGitHub`，脚本会自动走 API 下载。
...
- **v3.3**（2026-08-08）：**引导协议写死可执行判据，产出 Skill 内置执行层防御**——吸收真实 agent 实测反馈：①「能解析的绝不问」从建议升级为硬判据（0.5 节给出必问 / 必不问 / 必预填清单 + 发问卷前自检），防止 agent 退化成甩固定问卷；②产出 Skill 的 `ppt_apply.py` 强制内置执行层防御：XML 层表格行增删（Rows.Paste 不可靠）、rowSpan 纵向合并感知、改文本清多余 run、COM 绝对路径 + 3 次重试；③安装脚本新增 `-FromGitHub` 模式（GitHub API blob 兜底，网络受限时 git clone / raw 走不通也能装）。
